...
func ConvertContainer(container *parser.UnitFile, unitsInfoMap map[string]*UnitInfo, isUser bool) (*parser.UnitFile, error, error, []*parser.UnitFile) {
	...
	if len(image) == 0 && len(rootfs) == 0 {
		return nil, warnings, errors.New("no Image or Rootfs key specified"), nil
	}
	if len(image) > 0 && len(rootfs) > 0 {
		return nil, warnings, errors.New("the Image And Rootfs keys conflict can not be specified together"), nil
	}
